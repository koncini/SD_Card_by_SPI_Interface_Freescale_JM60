...
volatile static UINT16 u16FAT_Root_BASE;
...
void FAT_LS(void)
{ 
	UINT8 u8numberfiles=0;
    UINT8 u8Counter;
    static unsigned int i=0,j=0;
    static unsigned char sendbuf[64];
    root_Entries *sFileStructure;                                   
    for(i=0;i<=30;i++) {//cómo saber que ya no hay archivos 512 entries
		u8numberfiles=0;
		GetPhysicalBlock(u16FAT_Root_BASE+i,ag8FATReadBuffer);
		  
	   //sFileStructure = (root_Entries*)&ag8FATReadBuffer[RootEntrySize]; COMENTADO Y CAMBIADO POR LA LINEA 127 para tener en cuenta el primer archivo
		sFileStructure = (root_Entries*)&ag8FATReadBuffer[0];
		while(sFileStructure->FileName[0]!=FILE_Clear && u8numberfiles<15)//restriccion par lectura del archivo 16*32=512
		{																
			if(sFileStructure->FileName[0]!=FILE_Erased)
			{
				...
				for(u8Counter=0;u8Counter<8;u8Counter++){
					if(sFileStructure->FileName[u8Counter]!=' '){
						sendbuf[j++]=sFileStructure->FileName[u8Counter];
					}
				}
				sendbuf[j++]='.';
				for(u8Counter=0;u8Counter<3;u8Counter++){
					if(sFileStructure->Extension[u8Counter]!=' '){
						sendbuf[j++]=sFileStructure->Extension[u8Counter];
					}
				}
						//SendString2USB(sendbuf,j);
						//j=0;
						//Watchdog_Reset();
					//if(USB_PROCESS_PENDING()){USB_Engine();}					
                ...
                if(sFileStructure->FileName[0] == u8FileName[0])
                {
                    ...
                    if(u8Flag==True)
                    {
                        ...
                        else if (u8Function==OVERWRITE)
                        {
                            ...
                            u8Counter=0;
                            u8ErrorCode=(UINT8)u16FAT_Cluster_Size;
                       ...
                       /* If Modify Function */
                        else
                        {
                            ...
                            u8Counter=0;
                            u8ErrorCode=(UINT8)u16FAT_Cluster_Size; //modificado 21-06-13
                            while(u8ErrorCode != 0x01)
                            {
                                u8Counter++;
                                u8ErrorCode=u8ErrorCode>>1;
