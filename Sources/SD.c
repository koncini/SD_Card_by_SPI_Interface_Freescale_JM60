...
UINT8 SD_Init8MHz(void) 
{
    ...
    SPI1_SS=DISABLE;
    (void)SPI1_Receive_byte();  // Dummy SPI cycle
...
UINT8 SD_Write_Block(UINT32 u16SD_Block,UINT8 *pu8DataPointer) 
{
    ...
    for(u16Counter=0;u16Counter<BLOCK_SIZE;u16Counter++)
        SPI1_Send_byte(*pu8DataPointer++);

    SPI1_Send_byte(0xFF);    // checksum Bytes not needed
    SPI1_Send_byte(0xFF);

    for(u16Counter=0;u16Counter<BLOCK_SIZE;u16Counter++);
    
    if((SPI1_Receive_byte() & 0x0F) != 0x05)
    {
        SPI1_SS=DISABLE;
        return(WRITE_DATA_FAILS);      
    }

    while(SPI1_Receive_byte()==0x00);  // Dummy SPI cycle
...
UINT8 SD_Read_Block(UINT32 u16SD_Block,UINT8 *pu8DataPointer) 
{
    ...
    while(u8Temp!=0xFE)
        u8Temp=SPI1_Receive_byte();
    
    for(u16Counter=0;u16Counter<BLOCK_SIZE;u16Counter++)
        *pu8DataPointer++=SPI1_Receive_byte();

    (void)SPI1_Receive_byte();  // Dummy SPI cycle
    (void)SPI1_Receive_byte();  
...
UINT8 SD_SendCommand(UINT8 u8SDCommand,UINT8 u8SDResponse) 
{
    ...
    /* Send Start byte */
    SPI1_Send_byte(u8SDCommand);

    /* Send Argument */
    for(u8Counter=0;u8Counter<4;u8Counter++) 
        SPI1_Send_byte(gu8SD_Argument.bytes[u8Counter]);
